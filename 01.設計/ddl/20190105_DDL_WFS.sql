...
-- Date/Time    : 2019/01/05 11:51:48
-- Author       : akihiro Iwata
-- RDBMS Type   : PostgreSQL
-- Application  : A5:SQL Mk-2

-- ディーラー詳細作品分野
drop table if exists dealers_detail_products_categories cascade;

create table dealers_detail_products_categories (
  dealer_id integer not null
  , product_id integer
  , category_id integer not null
  , constraint dealers_detail_products_categories_PKC primary key (dealer_id,product_id,category_id)
) ;
...
create table categories (
  ...
  , category_name_reading varchar(50)
...
create table dealers_detail_products_imgs (
  ...
  , constraint dealers_detail_products_imgs_PKC primary key (dealer_id,product_id,img_seq)
) ;
...
alter table dealers_detail_products_categories
  add constraint dealers_detail_products_categories_FK1 foreign key (category_id) references categories(category_id);

alter table dealers_detail_products_categories
  add constraint dealers_detail_products_categories_FK2 foreign key (dealer_id,product_id) references dealers_detail_products(dealer_id,product_id);
...
comment on table dealers_detail_products_categories is 'ディーラー詳細作品分野';
comment on column dealers_detail_products_categories.dealer_id is 'ディーラーID';
comment on column dealers_detail_products_categories.product_id is '作品ID';
comment on column dealers_detail_products_categories.category_id is 'ジャンル';
...
comment on column categories.category_name_reading is '作品名読み';
...
comment on column dealers_detail_products_imgs.img_seq is '番号';
